...
for url in url_data:
    try:
        ...
        head_tag = soup.head
        date_publist_list.append(datetime.datetime.fromtimestamp(float(head_tag.find(attrs = {"property":"bt:pubDate"})['content'])/1000))
    except:
...
#     print(ls)
print("-------------------------------------------------------------------------------------------------------------------------")
print(len(title_list),len(creator_list),len(size_prod_list),len(views_list),len(favourites_list),len(price_list),len(medium_list),len(date_publist_list))
for d in (date_publist_list):
    print((d))
